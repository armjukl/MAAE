...
实现位于 `fake_device.py` 和 `fake_device_placeholder.py`。二者的 ADB 协议与输入逻辑相同，仅截图返回策略不同。
...
`fake_device.py` 每次收到截图请求时，都返回最新的浏览器截图缓存。适合优先保证识别画面新鲜度的场景。

### `placeholder`

`fake_device_placeholder.py` 每秒最多返回一次实时截图，其余请求返回 `MAAE_PLACEHOLDER_IMAGE` 指向的 PNG。若未配置或读取失败，则使用内置空白 PNG。该模式用于比较不同截图返回策略对 MaaCore 的影响。
