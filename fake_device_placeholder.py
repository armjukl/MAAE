...
def _screencap():
    """1秒内只给一次真实截图，其余返回占位图——对照实验。"""
    global _last_real_screencap_time
    with _screencap_lock:
        data = _cached_screencap
    now = time.time()
    if data and (now - _last_real_screencap_time) >= 1:
        _last_real_screencap_time = now
    ...
    _log.info(f"[screencap] placeholder: {len(placeholder)} bytes")
    return placeholder
